	...
	Creature(), lastPing(OTSYS_TIME()), lastPong(lastPing), inbox(new Inbox(ITEM_INBOX)), storeInbox(new StoreInbox(ITEM_STORE_INBOX)), client(std::move(p))
{
	...
	// ADICIONE NO FINAL DO CONSTRUTOR:
    skillTree = nullptr;
}
...
{
    if (!skillTree) {
        skillTree = std::make_unique<PlayerSkillTree>(this);
    }
}

bool Player::loadSkillTreeFromDatabase()
{
    if (!skillTree) {
        initSkillTree();
    }
    return skillTree->loadFromDatabase();
}

void Player::onSkillTreeLevelUp()
{
    if (skillTree && getLevel() >= SKILLTREE_MIN_LEVEL) {
        skillTree->updatePointsBasedOnLevel();
    }
}

int32_t Player::getSkillTreeAbsorption(CombatType_t combatType) const
{
    auto it = skillTreeAbsorptions.find(combatType);
    return it != skillTreeAbsorptions.end() ? it->second : 0;
}

void Player::setSkillTreeAbsorption(CombatType_t combatType, int32_t value)
{
    skillTreeAbsorptions[combatType] = value;
}

int32_t Player::getSkillTreeDamageBonus(CombatType_t combatType) const
{
    auto it = skillTreeDamageBonus.find(combatType);
    return it != skillTreeDamageBonus.end() ? it->second : 0;
}

void Player::setSkillTreeDamageBonus(CombatType_t combatType, int32_t value)
{
    skillTreeDamageBonus[combatType] = value;
}
