...
// =====================================================
// SKILL TREE FUNCTIONS - VERSÃO FINAL
// =====================================================

void ProtocolGame::sendExtendedOpcode(uint8_t opcode, const std::string& buffer)
{
    NetworkMessage msg;
    msg.addByte(0x32); // Extended Opcode
    msg.addByte(opcode);
    msg.addString(buffer);
    writeToOutputBuffer(msg);
}

void ProtocolGame::sendSkillTreeOpen()
{
    if (!player) {
        return;
    }

    PlayerSkillTree* skillTree = player->getSkillTree();
    if (!skillTree) {
        return;
    }
    ...
    msg.add<uint32_t>(skillTree->getAvailablePoints());
    msg.add<uint32_t>(skillTree->getTotalPointsEarned());
    msg.add<uint32_t>(skillTree->getPointsSpent());

    const auto& allNodes = SkillTreeManager::getInstance().getAllNodes();
    msg.add<uint16_t>(allNodes.size());

    for (const auto& nodePair : allNodes) {
        const SkillTreeNode& node = nodePair.second;

        msg.add<uint32_t>(node.getNodeId());
        msg.addString(node.getNodeName());
        msg.addByte(static_cast<uint8_t>(node.getNodeType()));
        msg.addDouble(node.getPositionX());
        msg.addDouble(node.getPositionY());
        msg.add<uint32_t>(node.getSpriteId());
        msg.addString(node.getDescription());
        msg.add<uint32_t>(node.getRequiredLevel());
        msg.addByte(node.isStarting() ? 1 : 0);
        msg.addByte(skillTree->hasNodeAllocated(node.getNodeId()) ? 1 : 0);

        const auto& connections = node.getConnections();
        msg.add<uint16_t>(connections.size());
        for (uint32_t connectedNodeId : connections) {
            msg.add<uint32_t>(connectedNodeId);
        }

        std::vector<SkillTreeBonus> bonuses = node.getBonusesForVocation(player->getVocationId());
        msg.add<uint16_t>(bonuses.size());
        for (const auto& bonus : bonuses) {
            msg.addByte(static_cast<uint8_t>(bonus.getBonusType()));
            msg.add<int32_t>(bonus.getBonusValue());
            msg.addByte(bonus.isPercentageBonus() ? 1 : 0);
        }
    }

    sendExtendedOpcode(240, std::string(reinterpret_cast<const char*>(msg.getBuffer()), msg.getLength()));
}

void ProtocolGame::sendSkillTreeUpdate(uint32_t nodeId, bool isAllocated)
{
    NetworkMessage msg;
    msg.add<uint32_t>(nodeId);
    msg.addByte(isAllocated ? 1 : 0);
    sendExtendedOpcode(241, std::string(reinterpret_cast<const char*>(msg.getBuffer()), msg.getLength()));
}

void ProtocolGame::sendSkillTreePoints(uint32_t availablePoints, uint32_t totalPoints, uint32_t spentPoints)
{
    NetworkMessage msg;
    msg.add<uint32_t>(availablePoints);
    msg.add<uint32_t>(totalPoints);
    msg.add<uint32_t>(spentPoints);
    sendExtendedOpcode(242, std::string(reinterpret_cast<const char*>(msg.getBuffer()), msg.getLength()));
}

void ProtocolGame::sendSkillTreeNodeAllocated(uint32_t nodeId, bool success, const std::string& message)
{
    NetworkMessage msg;
    msg.add<uint32_t>(nodeId);
    msg.addByte(success ? 1 : 0);
    msg.addString(message);
    sendExtendedOpcode(243, std::string(reinterpret_cast<const char*>(msg.getBuffer()), msg.getLength()));
}

void ProtocolGame::sendSkillTreeReset(bool success, const std::string& message)
{
    NetworkMessage msg;
    msg.addByte(success ? 1 : 0);
    msg.addString(message);
    sendExtendedOpcode(244, std::string(reinterpret_cast<const char*>(msg.getBuffer()), msg.getLength()));
}

void ProtocolGame::parseSkillTreeRequest(const std::string& /*buffer*/)
{
    if (!player || player->getLevel() < 100) {
        return;
    }
    sendSkillTreeOpen();
}

void ProtocolGame::parseSkillTreeAllocate(const std::string& buffer)
{
    if (!player) {
        return;
    }
    ...
    uint32_t nodeId;
    memcpy(&nodeId, buffer.data(), sizeof(uint32_t));

    bool success = skillTree->allocateNode(nodeId);

    if (success) {
        sendSkillTreeNodeAllocated(nodeId, true, "Node allocated!");
        sendSkillTreeUpdate(nodeId, true);
        sendSkillTreePoints(
            skillTree->getAvailablePoints(),
            skillTree->getTotalPointsEarned(),
            skillTree->getPointsSpent()
        );
    } else {
        sendSkillTreeNodeAllocated(nodeId, false, "Failed to allocate.");
    }
}

void ProtocolGame::parseSkillTreeDeallocate(const std::string& buffer)
{
    if (!player) {
        return;
    }
    ...
    if (skillTree->deallocateNode(nodeId)) {
        sendSkillTreeUpdate(nodeId, false);
        sendSkillTreePoints(
            skillTree->getAvailablePoints(),
            skillTree->getTotalPointsEarned(),
            skillTree->getPointsSpent()
...
void ProtocolGame::parseSkillTreeResetRequest(const std::string& /*buffer*/)
{
    if (!player) {
        return;
    }
    ...
    uint64_t paidCost = skillTree->resetTree();

    if (paidCost > 0) {
        sendSkillTreeReset(true, "Skill Tree reset!");
        sendSkillTreeOpen();
    } else {
        sendSkillTreeReset(false, "Not enough gold.");
    }
}

// =====================================================
// FIM DAS FUNÇÕES SKILL TREE
// =====================================================
...
void ProtocolGame::parseExtendedOpcode(NetworkMessage& msg)
{
	...
	switch(opcode) {
        case 240: parseSkillTreeRequest(buffer); return;
        case 241: parseSkillTreeAllocate(buffer); return;
        case 242: parseSkillTreeDeallocate(buffer); return;
        case 243: parseSkillTreeResetRequest(buffer); return;
        default: break;
    }
