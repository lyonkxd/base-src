...
#include "weapons.h"
#include "script.h"
#include "skilltree.h" // <<< ADICIONE ESTA LINHA AQUI
...
void Game::start(ServiceManager* manager)
{
	...
	// ===== INÍCIO DA MODIFICAÇÃO DA SKILL TREE =====
	std::cout << ">> Loading Skill Tree System..." << std::endl;
	if (!SkillTreeManager::getInstance().loadFromDatabase()) {
		std::cout << "FATAL ERROR: Failed to load Skill Tree system!" << std::endl;
		// Interrompe a inicialização se o sistema de skill tree não carregar
		return;
	}
	// ===== FIM DA MODIFICAÇÃO =====
}
